...
#define I2C_BUS_MUTEX_TAKE(mutex, ret)                                         \
	if (!xSemaphoreTake(mutex, I2C_BUS_MUTEX_TICKS_TO_WAIT)) {                 \
		ESP_LOGE(TAG,                                                          \
		         "i2c_bus take mutex timeout, max wait = %d ms",               \
...
esp_err_t i2c_bus_write_reg8(i2c_bus_device_handle_t dev_handle,
                             uint8_t mem_address,
                             size_t data_len,
                             const uint8_t *data);
esp_err_t i2c_bus_read_reg8(i2c_bus_device_handle_t dev_handle,
                            uint8_t mem_address,
                            size_t data_len,
                            uint8_t *data);
...
i2c_bus_handle_t
i2c_bus_create(i2c_port_t port, const i2c_config_t *conf)
{
	...
	I2C_BUS_CHECK(conf->mode == I2C_MODE_MASTER,
	              "i2c_bus only supports master mode",
	              NULL);
	...
	if (s_i2c_bus[port].is_init) {
		...
		if (i2c_config_compare(port, conf)) {
			ESP_LOGW(
			    TAG,
			    "i2c%d has been inited, return handle directly, ref_counter=%d",
			    port,
		...
		I2C_BUS_CHECK(s_i2c_bus[port].mutex != NULL,
		              "i2c_bus xSemaphoreCreateMutex failed",
		              NULL);
...
esp_err_t
i2c_bus_delete(i2c_bus_handle_t *p_bus)
{
	I2C_BUS_CHECK(p_bus != NULL && *p_bus != NULL,
	              "pointer = NULL error",
	              ESP_ERR_INVALID_ARG);
	...
	if ((i2c_bus->ref_counter) > 0) {
		ESP_LOGW(TAG,
		         "i2c%d is also handled by others ref_counter=%u, won't be "
		         "de-inited",
		         i2c_bus->i2c_port,
		         i2c_bus->ref_counter);
		return ESP_OK;
...
i2c_bus_scan(i2c_bus_handle_t bus_handle, uint8_t *buf, uint8_t num)
{
	...
	for (uint8_t dev_address = 1; dev_address < 127; dev_address++) {
		...
		i2c_master_write_byte(
		    cmd, (dev_address << 1) | I2C_MASTER_WRITE, I2C_ACK_CHECK_EN);
		i2c_master_stop(cmd);
		esp_err_t ret =
...
uint32_t
i2c_bus_get_current_clk_speed(i2c_bus_handle_t bus_handle)
{
...
uint8_t
i2c_bus_get_created_device_num(i2c_bus_handle_t bus_handle)
{
	I2C_BUS_CHECK(bus_handle != NULL, "Null Bus Handle", 0);
	i2c_bus_t *i2c_bus = (i2c_bus_t *)bus_handle;
	I2C_BUS_INIT_CHECK(i2c_bus->is_init, 0);
...
i2c_bus_device_create(i2c_bus_handle_t bus_handle,
                      uint8_t dev_addr,
                      uint32_t clk_speed)
{
...
esp_err_t
i2c_bus_device_delete(i2c_bus_device_handle_t *p_dev_handle)
{
...
uint8_t
i2c_bus_device_get_address(i2c_bus_device_handle_t dev_handle)
{
...
esp_err_t
i2c_bus_read_bytes(i2c_bus_device_handle_t dev_handle,
                   uint8_t mem_address,
                   size_t data_len,
                   uint8_t *data)
{
...
esp_err_t
i2c_bus_read_byte(i2c_bus_device_handle_t dev_handle,
                  uint8_t mem_address,
                  uint8_t *data)
{
...
esp_err_t
i2c_bus_read_bit(i2c_bus_device_handle_t dev_handle,
                 uint8_t mem_address,
                 uint8_t bit_num,
                 uint8_t *data)
{
...
esp_err_t
i2c_bus_read_bits(i2c_bus_device_handle_t dev_handle,
                  uint8_t mem_address,
                  uint8_t bit_start,
                  uint8_t length,
                  uint8_t *data)
...
esp_err_t
i2c_bus_write_byte(i2c_bus_device_handle_t dev_handle,
                   uint8_t mem_address,
                   uint8_t data)
{
...
esp_err_t
i2c_bus_write_bytes(i2c_bus_device_handle_t dev_handle,
                    uint8_t mem_address,
                    size_t data_len,
                    const uint8_t *data)
{
...
esp_err_t
i2c_bus_write_bit(i2c_bus_device_handle_t dev_handle,
                  uint8_t mem_address,
                  uint8_t bit_num,
                  uint8_t data)
{
...
esp_err_t
i2c_bus_write_bits(i2c_bus_device_handle_t dev_handle,
                   uint8_t mem_address,
                   uint8_t bit_start,
                   uint8_t length,
                   uint8_t data)
...
esp_err_t
i2c_bus_read_word(i2c_bus_device_handle_t dev_handle,
                  uint8_t mem_address,
                  uint16_t *data)
{
	uint8_t msb[2] = {0, 0};
	esp_err_t rc = i2c_bus_read_bytes(dev_handle, mem_address, 2, msb);
	*data = (int16_t)((msb[0] << 8) | msb[1]);
	return rc;
}

esp_err_t
i2c_bus_write_word(i2c_bus_device_handle_t dev_handle,
                   uint8_t mem_address,
                   uint16_t data)
{
	uint8_t data1[] = {(uint8_t)(data >> 8), (uint8_t)(data & 0xff)};
	esp_err_t rc = i2c_bus_write_bytes(dev_handle, mem_address, 2, data1);
	return rc;
}
 ...
 * @return esp_err_t
 */
inline static esp_err_t
i2c_master_cmd_begin_with_conf(i2c_port_t i2c_num,
...
esp_err_t
i2c_bus_cmd_begin(i2c_bus_device_handle_t dev_handle, i2c_cmd_handle_t cmd)
{
	I2C_BUS_CHECK(
	    dev_handle != NULL, "device handle error", ESP_ERR_INVALID_ARG);
	...
	esp_err_t ret =
	    i2c_master_cmd_begin_with_conf(i2c_device->i2c_bus->i2c_port,
	                                   cmd,
...
esp_err_t
i2c_bus_read_reg8(i2c_bus_device_handle_t dev_handle,
                  uint8_t mem_address,
                  size_t data_len,
                  uint8_t *data)
{
	...
	if (mem_address != NULL_I2C_MEM_ADDR) {
		...
		i2c_master_write_byte(cmd,
		                      (i2c_device->dev_addr << 1) | I2C_MASTER_WRITE,
		                      I2C_ACK_CHECK_EN);
	...
	i2c_master_start(cmd);
	i2c_master_write_byte(
	    cmd, (i2c_device->dev_addr << 1) | I2C_MASTER_READ, I2C_ACK_CHECK_EN);
...
esp_err_t
i2c_bus_read_reg16(i2c_bus_device_handle_t dev_handle,
...
esp_err_t
i2c_bus_write_reg8(i2c_bus_device_handle_t dev_handle,
                   uint8_t mem_address,
                   size_t data_len,
                   const uint8_t *data)
{
	...
	i2c_master_start(cmd);
	i2c_master_write_byte(
	    cmd, (i2c_device->dev_addr << 1) | I2C_MASTER_WRITE, I2C_ACK_CHECK_EN);
...
esp_err_t
i2c_bus_write_reg16(i2c_bus_device_handle_t dev_handle,
                    uint16_t mem_address,
                    size_t data_len,
                    const uint8_t *data)
{
...
/* Private Functions */
static esp_err_t
i2c_driver_reinit(i2c_port_t port, const i2c_config_t *conf)
{
	...
	ret = i2c_driver_install(port,
	                         conf->mode,
	                         I2C_BUS_MASTER_BUF_LEN,
	                         I2C_BUS_MASTER_BUF_LEN,
	                         I2C_BUS_FLG_DEFAULT);
...
static esp_err_t
i2c_driver_deinit(i2c_port_t port)
{
	I2C_BUS_CHECK(port < I2C_NUM_MAX, "i2c port error", ESP_ERR_INVALID_ARG);
	I2C_BUS_CHECK(s_i2c_bus[port].is_init == true,
	              "i2c not inited",
 ...
 */
inline static bool
i2c_config_compare(i2c_port_t port, const i2c_config_t *conf)
{
	if (s_i2c_bus[port].conf_active.master.clk_speed ==
